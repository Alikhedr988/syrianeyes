    ...
    // output data of each row
        ...
        $album_folder = str_replace(' ', '_', $ph_album_title);   
           ...
           $full_dir = $directory . $album_folder ;
            $ph_album_desc = strip_tags($ph_album_desc);
          if (is_dir($full_dir)) {
    if ($dh = opendir($full_dir)) {
        ...
        while (($file = readdir($dh)) !== false) {
            if (!is_dir($full_dir.$file)) {
                 $album_photos .= '<a class="fancybox photo-gallery-hide" href="'.$dir_from_index.$album_folder . '/' .$file.'" data-fancybox-group="gallery'.$i.'" title="'.$ph_album_desc.'"></a>';
           ...
           {    
            ...
            <div class="grid">
                <figure class="effect-oscar">
                    ...
                    <figcaption>
                        <h2>'.$ph_album_title.'</h2>
                        
                        <a class="fancybox cover-photo-gallery" href="includes/uploads/p-cover/'.$ph_album_cover.'" data-fancybox-group="gallery'.$i.'" title="'.$ph_album_desc.'"></a>
                    </figcaption>
                </figure>
...
<div class="row gallery-row-container">
    ...
    <div class="col-md-11 col-xs-10">
        <h1 class="text-center gallery-title"> PHOTO GALLERY</h1>
    </div>
</div>
<?=$albums_container?>